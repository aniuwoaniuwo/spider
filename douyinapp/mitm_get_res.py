    ...
    with open('url.text','w') as f:
        if '/aweme/v1/user/follower/list/' in flow.request.url:
            for follower in json.loads(flow.response.text)['followers']:
                data['name']=follower['nickname']
                data['id']=follower['short_id']#不是unique_id
                data['share_id']=follower['uid']
                print(data)
                save_database.base(data)
            # f.write(flow.response.text)


#mitmdump -p 8889 -s test.py进行启动,不过先进去到相应的路径下
