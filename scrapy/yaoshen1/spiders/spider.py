#-*-coding:utf-8-*-
#scrapy爬虫实际运用了异步的方法进而加快爬取的进度
#这个爬虫模板也增加了headers，代理还有cookies
#储存方法有txt，excel，数据库等方式
...
class yaoshen(scrapy.Spider):
    ...
    def start_requests(self):
        for url in self.start_urls:#加上self..下面那里加了ip起作用了，并且mid也添加了，是mid的优先
            yield scrapy.Request(url,callback=self.parse,cookies=self.cookies)#,meta=self.headers可以添加头，但是不能添加proxies和cookie，因为没用get的功能,用到meta的方法添加proxies要用到callback=self.parse,否则获取不了网页
